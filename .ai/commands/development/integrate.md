...
description: "Connect services, APIs, databases, and tools to create seamless integrations with proper error handling and monitoring"
target: "claude-code"
...
claudeCode:
  ...
  arguments:
    ...
    examples: ["api with database", "webhook with queue", "oauth with service", "--type rest", "--monitor"]
  fileReferences:
    supports: true
    autoInclude: ["CLAUDE.md", "package.json", ".env", "docker-compose.yml", "config/"]
  bashCommands:
    supports: true
    commands: ["curl", "docker", "deno", "node", "go", "mvn"]
  mcpIntegration:
    requiredServers: []
    optionalServers: ["database", "monitoring", "auth-provider"]
...
examples:
  - usage: "/integrate api with database"
    description: "Create REST API integration with database including caching and error handling"
    context: "Building microservice that needs to sync data between external API and local database"
    expectedOutcome: "API client, database layer, caching, retry logic, and monitoring integration"
  - usage: "/integrate webhook with queue"
    description: "Set up webhook receiver that publishes events to message queue"
    context: "Processing third-party webhooks asynchronously with reliable message handling"
    expectedOutcome: "Webhook endpoint, message queue publisher, signature verification, and event processing"
...
# === Installation ===
installation:
  dependencies: ["deno", "integration libraries", "monitoring tools"]
  setupSteps:
